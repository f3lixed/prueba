      ...
      link: function(scope, elm, attrs, controllers) {
        ...
        scope.checkAll = function () {
            var selected = scope.selected,
                availables = selected.available,
                i = 0, available;
            for (; available = availables[i]; i++) {
                available.selected = selected.control;
            }
        };
        ...
        scope.add = function() {
          if(!scope.model.length)
            scope.model = [];
          scope.$parent.$items = scope.selected(scope.selected.available);
          scope.model = scope.model.concat(scope.$parent.$items);
          scope.onAdd();
        };
        scope.remove = function() {
          scope.$parent.$items = scope.selected(scope.selected.current);
          scope.available = scope.available.concat(scope.$parent.$items);
          scope.model = filterOut(scope.model, scope.$parent.$items);
          scope.onRemove();
        };
        ...
        scope.renderDisabled = function(item) {
            if(attrs.disabled) {
                return parseExpression(item, attrs.disabled);
            }
            return false;
        }
  ...
  angular.module("template/multiSelect.html", []).run(["$templateCache", function($templateCache) {
    $templateCache.put("template/multiSelect.html",
      '<div class="multiSelect">' +
          ...
          '<ul>' +
            '<li ng-repeat="entity in model">' +
              '<label class="checkbox" title="{{ renderTitle(entity) }}">' +
                '<input type="checkbox" ng-model="selected.current[$index].selected" ng-disabled="renderDisabled(entity)" /> ' +
                '{{ renderItem(entity) }}' +
              '</label>' +
            '</li>' +
          '</ul>' +
        ...
        '</div>' +
        '<div class="select">' +
          '<label><input type="checkbox" style="vertical-align:top;margin-left:10px" ng-model="selected.control"  ng-change="checkAll()" /> Todos</label>' +
          '<ul>' +
            '<li ng-repeat="entity in available">' +
              '<label class="checkbox" title="{{ renderTitle(entity) }}">' +
                '<input type="checkbox" ng-model="selected.available[$index].selected" ng-disabled="renderDisabled(entity)" /> ' +
                '{{ renderItem(entity) }}' +
              '</label>' +
            '</li>' +
          '</ul>' +
